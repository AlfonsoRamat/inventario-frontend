...
export default function CardGrafico_caja() {
    ...
    function llenar_turno_tarde() {
        Cajas.forEach((Caja) => {
            if
                (Caja.turno.toString().toLowerCase().indexOf("TARDE".toLowerCase()) > -1) { fecha_turno_tarde.push(Caja.fecha) }

            if
                (Caja.turno.toString().toLowerCase().indexOf("TARDE".toLowerCase()) > -1) { venta_turno_tarde.push(Caja.montoTotalVendido) }
        });
    ...
    function llenar_turno_mañana() {
        Cajas.forEach((Caja) => {
            if((Caja.turno.toString().toLowerCase().indexOf("MAÑANA".toLowerCase()) > -1))
                 { fecha_turno_mañana.push(Caja.fecha) }
